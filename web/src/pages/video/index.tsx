...
function isVideoInAssetLibrary(assets: Asset[], video: GeneratedVideo) {
    return assets.some((asset) => asset.kind === "video" && (asset.data.storageKey === video.storageKey || asset.metadata?.generatedVideoId === video.id));
}
...
export default function VideoPage() {
    ...
    const saveResultToAssets = (video: GeneratedVideo, videoPrompt = prompt, notify = true) => {
        const exists = isVideoInAssetLibrary(assets, video);
        if (exists) {
            if (notify) message.info("视频已在我的素材库中");
            return;
        }
        addAsset({
            kind: "video",
            title: videoPrompt.slice(0, 24) || "生成视频",
            coverUrl: "",
            tags: [],
            source: "视频创作台",
            data: { url: video.url, storageKey: video.storageKey, width: video.width, height: video.height, bytes: video.bytes, mimeType: video.mimeType },
            metadata: { source: "video-page", generatedVideoId: video.id, prompt: videoPrompt },
        });
        if (notify) message.success("已加入我的素材库");
    };
        ...
        try {
            for (let attempt = 0; attempt < 120; attempt += 1) {
                ...
                if (state.status === "completed") {
                    ...
                    saveResultToAssets(nextVideo, log.prompt, false);
    ...
    return (
        <div className="specular-ui video-surface flex h-full flex-col overflow-hidden bg-stone-50 text-stone-900 dark:bg-[#111111] dark:text-stone-100">
                    ...
                    <div className="specular-frame thin-scrollbar rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto lg:p-5">
                        ...
                        {results.length ? (
                            <div className="grid gap-4">
                                {results.map((result) => (result.status === "success" && result.video ? <ResultVideoCard key={result.id} video={result.video} saved={isVideoInAssetLibrary(assets, result.video)} onDownload={downloadVideo} onSaveAsset={saveResultToAssets} /> : result.status === "failed" ? <FailedVideoCard key={result.id} error={result.error || "生成失败"} onRetry={retryResult} /> : <PendingVideoCard key={result.id} />))}
                            </div>
                        ) : (
...
function ResultVideoCard({ video, saved, onDownload, onSaveAsset }: { video: GeneratedVideo; saved: boolean; onDownload: (video: GeneratedVideo) => void; onSaveAsset: (video: GeneratedVideo) => void }) {
    return (
        <div className="overflow-hidden rounded-lg border border-stone-200 bg-background dark:border-stone-800">
            ...
            <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-2 border-t border-stone-200 px-3 py-2.5 dark:border-stone-800">
                ...
                <div className="flex shrink-0 gap-1">
                    <Button size="small" icon={<FolderPlus className="size-3.5" />} disabled={saved} onClick={() => onSaveAsset(video)}>
                        {saved ? "\u5df2\u4fdd\u5b58\u81f3\u7d20\u6750\u5e93" : "\u6dfb\u52a0\u5230\u7d20\u6750\u5e93"}
                    </Button>
...
function LogCard({ log, selected, active, onSelectedChange, onClick }: { log: GenerationLog; selected: boolean; active: boolean; onSelectedChange: (checked: boolean) => void; onClick: () => void }) {
    return (
        <button type="button" className={`specular-frame block w-full rounded-lg border p-2 text-left transition ${active ? "border-stone-900 bg-blue-50 dark:border-stone-100 dark:bg-blue-950/20" : "border-stone-200 bg-background hover:bg-stone-50 dark:border-stone-800 dark:hover:bg-stone-900"}`} onClick={onClick}>
