...
export function AssetPickerModal({ open, onInsert, onClose }: Props) {
    return (
        <Modal title="我的素材库" open={open} onCancel={onClose} footer={null} width={860} destroyOnHidden styles={{ body: { padding: "0 24px 24px", minHeight: 480 } }}>
...
function PickerCard({ asset, onClick }: { asset: Asset; onClick: () => void }) {
    ...
    const cover = asset.coverUrl || (kind === "image" ? asset.data.dataUrl : "");
    return (
        <button
            type="button"
        ...
        >
            {kind === "video" ? (
                <video src={asset.data.url} muted preload="metadata" className="aspect-[4/3] w-full bg-black object-cover" />
            ) : cover ? (
