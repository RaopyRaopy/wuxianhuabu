...
export async function requestVideoGeneration(config: AiConfig, prompt: string, references: ReferenceImage[] = [], videoReferences: ReferenceVideo[] = [], audioReferences: ReferenceAudio[] = [], options?: RequestOptions): Promise<VideoGenerationResult> {
    ...
    const delayMs = task.provider === "seedance" || isGrokVideoModel(task.model) ? 5000 : 2500;
    const maxAttempts = task.provider === "openai" && isGrokVideoModel(task.model) ? 180 : 120;
...
async function createOpenAIVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: RequestOptions): Promise<VideoGenerationTask> {
    if (isGrokVideoModel(model)) return createGrokVideoTask(config, model, prompt, references, options);
...
async function createGrokVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: RequestOptions): Promise<VideoGenerationTask> {
    const normalizedResolution = normalizeGrokResolution(config.vquality);
    const image = references[0] ? { url: await imageToDataUrl(references[0]) } : undefined;
    if (normalizedResolution === "1080p" && !image) throw new Error("Grok 1080p 图生视频必须提供参考图片");
    const payload = {
        prompt: prompt.trim(),
    ...
    if (!payload.prompt) throw new Error("请输入视频提示词");
    try {
        const created = unwrapVideoResponse((await axios.post<ApiVideoResponse>(aiApiUrl(config, "/videos/generations/async"), payload, { headers: aiHeaders(config, "application/json"), signal: options?.signal })).data);
        if (!created.id) throw new Error("Grok 视频接口没有返回任务 ID");
        return { id: created.id, provider: "openai", model };
    } catch (error) {
...
async function pollOpenAIVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        ...
        if (["completed", "done", "succeeded"].includes(status)) {
            const resultUrl = video.metadata?.video_url || video.video?.url;
            if (resultUrl && !resultUrl.includes(`/videos/${encodeURIComponent(task.id)}/content`)) return { status: "completed", result: await videoResultFromUrl(resultUrl, options) };
        ...
        if (["failed", "cancelled", "expired"].includes(status)) return { status: "failed", error: video.error?.message || `视频生成${status === "expired" ? "超时" : "失败"}` };
...
function isGrokVideoModel(model: string) {
    return modelOptionName(model).toLowerCase().startsWith("grok-imagine-video");
}
...
function normalizeGrokResolution(value: string) {
    ...
    if (normalized === "480" || normalized === "720" || normalized === "1080") return `${normalized}p`;
...
function normalizeGrokAspectRatio(value: string) {
    ...
    if (!match) return ["1:1", "16:9", "9:16", "4:3", "3:4", "3:2", "2:3"].includes(value) ? value : "16:9";
    const ratio = Number(match[1]) / Number(match[2]);
    const options = [["16:9", 16 / 9], ["3:2", 3 / 2], ["4:3", 4 / 3], ["1:1", 1], ["3:4", 3 / 4], ["2:3", 2 / 3], ["9:16", 9 / 16]] as const;
